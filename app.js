(() => {
  ...
  function imageCandidates(p){
    const explicit = p.photoFileName || p.photoFile || p.image || "";
    const code = p.plantId || p.PlantID || "";
    const candidates = [];

    if(explicit){
      const clean = String(explicit).replace(/^assets\/plants\//,"");
      candidates.push(`assets/plants/${clean}`);
    }

    if(code){
      ["webp","jpg","jpeg","png"].forEach(ext => candidates.push(`assets/plants/${code}.${ext}`));
    }
  ...
  function loadPlantThumbnail(article,p){
    ...
    const img = article.querySelector(".plant-thumbnail");
    ...
    img.addEventListener("load", () => { banner.hidden = false; }, {once:false});
  ...
  function makeCard(p,score,hasCriteria){
    ...
    article.innerHTML=`
      <div class="card-banner" hidden>
        <img class="plant-thumbnail" alt="" loading="lazy" decoding="async">
      </div>
      <div class="card-body">
        ...
        <button class="details-button" type="button">Click here to see restoration + plant details <strong aria-hidden="true">→</strong></button>
      </div>`;

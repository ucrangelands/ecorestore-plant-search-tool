(() => {
  ...
  function intersectionCount(selected, available){
    let n=0;
    selected.forEach(v=>{
      if((available||[]).includes(v)) n++;
    });
    return n;
  }
  ...
  function scorePlant(p){
    ...
    if(state.ecosystem){
      ...
      (p.ecosystems||[]).includes(state.ecosystem)
        ? (earned+=weights.ecosystem,reasons.push(state.ecosystem))
        : misses.push(`Ecosystem: ${state.ecosystem}`);
    }
    ...
    if(state.county){
      ...
      (p.counties||[]).includes(state.county)
        ? (earned+=weights.county,reasons.push(`${state.county} County`))
    ...
    [
      ...
      if(selected.size){
        ...
        const matched=[...selected].filter(
          v=>(p[field]||[]).includes(v)
        );

        earned += w*matched.length;

        matched.forEach(v=>reasons.push(v));

        [...selected]
          .filter(v=>!matched.includes(v))
          .forEach(v=>misses.push(`${label}: ${v}`));
      }
    });
  ...
  /*
   * Build a list of possible locally hosted plant images.
   *
   * Images can be stored in:
   *
   * assets/plants/
   *
   * using the PlantID as the filename:
   *
   * ACMI.webp
   * ACMI.jpg
   * ACMI.jpeg
   * ACMI.png
   *
   * An explicit photo filename in the plant record takes priority.
   */
  ...
  /*
   * Try each possible image filename.
   *
   * If an image successfully loads, the thumbnail header is shown.
   * If no image exists, the image header remains hidden.
   */
  function loadPlantThumbnail(article,p){
    const banner =
      article.querySelector(".card-banner");

    const img =
      article.querySelector(".plant-thumbnail");
  ...
  function render(){
    ...
    rows.sort((a,b)=>
      state.sort==="common"
        ? a.p.common.localeCompare(b.p.common)

        : state.sort==="scientific"
        ? a.p.scientific.localeCompare(b.p.scientific)
    ...
    els.summary.textContent =
      hasCriteria
        ...
        : "Showing all plants in the starter dataset. Add site criteria to rank recommendations.";
    ...
    article.innerHTML=`
      ...
      <div class="card-body">
        ...
        <div class="meta-row">
          ...
          ${
            (p.services||[])
              .slice(0,2)
              .map(
                s=>`<span class="tag">${escapeHTML(s)}</span>`
              )
              .join("")
          }
        ...
        <div class="why">
          ...
          <strong>
            ${escapeHTML(
              matchLabel(score.pct,hasCriteria)
            )}
          </strong>

          ${
            hasCriteria && topReasons.length
  ...
  function openPlant(p,score,hasCriteria){
    ...
    [
      ["Grazing",state.grazing,p.grazing],
      ["Soil texture",state.soils,p.soils],
      ["Soil chemistry",state.chemistry,p.chemistry],
      ["Site conditions",state.conditions,p.conditions],
      ["Restoration goals",state.goals,p.goals]
    ].forEach(
      ([label,selected,available])=>{
      ...
      <section class="detail-section">
        ...
        ${
          rows.length
            ...
            : "<p>No site criteria are active. Close this window and choose site characteristics to generate an explanation.</p>"
        }
      ...
      <section class="detail-section">
        ...
        <div class="detail-list">
          ...
          ${
            (p.services||[])
              .map(
                x=>`<span class="tag">${escapeHTML(x)}</span>`
              )
              .join("")
          }
  ...
  function detailMatch(label,value,matched){
    return `
      <tr>
        <th>${escapeHTML(label)}</th>
        <td>
          <span class="${matched?"check":"miss"}">
            ${
              matched
                ? "✓ Matches"
